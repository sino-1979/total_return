...
def plot_two_panels(sale, sale_cum, div, div_cum, file_title):
    ...
    if not all_dates.empty:
        ...
        min_date, max_date = today - pd.Timedelta(days=1), today

    all_vals = np.array([])
    for s in [sale, sale_cum, div, div_cum]:
        if not s.empty:
            all_vals = np.concatenate([all_vals, s.values])
    if all_vals.size == 0:
        ylim = (-1000, 1000)
    else:
    ...
    if not sale_cum.empty:
        ...
        ax1.annotate(f"累計: ¥{y:,.0f}", xy=(x, y), xytext=(10, 10),
                     textcoords="offset points", fontsize=12, fontweight="bold",
                     bbox=dict(boxstyle="round,pad=0.5", fc="yellow", alpha=0.8))
    ax1.set_title("売却益（キャピタルゲイン）", fontsize=14)
    ...
    if not div_cum.empty:
        ...
        ax2.annotate(f"累計: ¥{y:,.0f}", xy=(x, y), xytext=(10, 10),
                     textcoords="offset points", fontsize=12, fontweight="bold",
                     bbox=dict(boxstyle="round,pad=0.5", fc="yellow", alpha=0.8))
    ax2.set_title("配当金（インカムゲイン）", fontsize=14)
    ax2.grid(axis="y", linestyle='--', alpha=0.6)
    ax2.axhline(0, color="black", linewidth=0.8)
    ax2.set_ylim(ylim)
    ax2.set_xlabel("受渡日", fontsize=12)
    ...
    fig.suptitle(f"【{file_title}】\n売却益・配当金（日次・累積純利益）",
                 fontsize=16, fontweight="bold", y=0.98)
    plt.tight_layout(rect=[0, 0.05, 1, 0.93])
    return fig

# ===== Streamlit UI =====
st.title("SBI証券 日次・累積損益グラフアプリ（全件表示・スマホ縦長対応）")
...
if uploaded_file is not None:
    ...
    for idx, row in enumerate(csv.reader(csv_io)):
        ...
        if len(row) == 12: rec12.append(row)
        elif len(row) == 13: rec13.append(row)
    ...
    div_daily, div_cum = make_daily_series(df_div, tax_div, pd.DataFrame(columns=["受渡日", "損益金額/徴収額", "地方税"]))
    all_sale = sale_cum.values[-1] if not sale_cum.empty else 0
    all_div = div_cum.values[-1] if not div_cum.empty else 0

    # 全件表示
    st.subheader("取引データ（全件）")
    st.dataframe(df)  # ここをhead()→dfで全行表示！

    fig = plot_two_panels(sale_daily, sale_cum, div_daily, div_cum, uploaded_file.name.replace('.csv', ''))
    st.pyplot(fig, use_container_width=True) # 横幅は画面に合わせて、縦はfigsizeで調整

    st.subheader("最終損益サマリー")
